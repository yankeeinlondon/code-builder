...
import code, { CodeOptions } from "../src/index";
import { composeFixture, getFixture } from "./utils";
import { getPrismGrammar } from "../src/mdi";
import { ConfiguredBuilder } from "@yankeeinlondon/builder-api";

describe("code() builder using Prism (incl generalized tests)", () => {
  it("code provides API with correct options hash", () => {
    const empty = code();
    const withParams = code({ theme: "dracula" });

    type Empty = typeof empty;
    type WithParams = typeof withParams;
    type Configured = ConfiguredBuilder<
      "code",
      Partial<CodeOptions>,
      "parser",
    ...
    type cases = [
      Expect<Equal<Empty, Configured>>,
      Expect<Equal<WithParams, Configured>>
    ];
    const test: cases = [true, true];
    expect(test).toEqual(test);
  });

  it("valid language choice is rendered", async () => {
    const { templateBlock } = await composeFixture("multi-code-block.md", {
      builders: [code()],
    });
  ...
  it('"unknown language" fallback is used when language stated but not matched', async () => {
    const { templateBlock } = await composeFixture("unknown-code-block", {
      builders: [code()],
    });
  ...
  it("the correct fallback is used when NO language is found", async () => {
    const { templateBlock } = await composeFixture("no-language", {
      builders: [
        code({
  ...
  it("line numbers are displayed when set", async () => {
    const { html, templateBlock } = await composeFixture("ts-code-block", {
      builders: [
        code({
          lineNumbers: true,
        }),
    ...
    expect(
      lineNumberMode.length,
      ".line-numbers-mode should be found once"
    ).toBe(1);
    expect(linesWrapper.length).toBe(1);
    expect(lineNumCols.length, ".line-numbers selector").toBe(5);
    expect(
      block.length,
      "the wrapping table for cols/line-numbers should be found"
    ).toBe(1);
  ...
  it("line highlighting of single line works using vuepress/vitepress syntax", async () => {
    ...
    const { html, templateBlock } = await composeFixture("highlight-one-line", {
    ...
    expect(
      highlighted.length,
      "both code and line number received a highlight class"
    ).toBe(2);
  ...
  it("line highlighting of line range works using vuepress/vitepress syntax", async () => {
    ...
    expect(highlighted.length, "there should be three lines highlighted").toBe(
      3
    );
    highlighted.map((el) =>
      expect(
        getClassList(el),
        'a highlighted line should also contain a "line" class'
      ).toContain("code-line")
    );
  ...
  it("line highlighting of line range works using object syntax", async () => {
    const { html } = await composeSfcBlocks(
      ...
      (
        await getFixture("highlight-one-line.md")
      ).replace("{4}", '{ "highlight": [2,3] }'),
      { builders: [code()] }
    );

    const highlighted = select(html).findAll(".highlight");
    expect(highlighted, "explicit array elements are highlighted").toHaveLength(
      2
    );
    for (const el of highlighted) {
      expect(getClassList(el)).toContain("code-line");
    }
    ...
    expect(highlighted2, "array syntax is interpreted correctly").toHaveLength(
      2
    );
    for (const el of highlighted2) {
      expect(getClassList(el)).toContain("code-line");
    }
  });
  ...
  it("code content loaded from file using <<< syntax", async () => {
    const { html } = await composeFixture("external-reference", {
      builders: [code()],
    });

    const sel = select(html);

    expect(getClassList(sel.findFirst(".code-block"))).toContain(
      "external-ref"
    );

    const comments = sel
      .mapAll(".comment")((c) => toHtml(c))
      .join("\n");
  ...
  it("highlighting imported code, while also using inline code", async () => {
    const { html } = await composeFixture("external-reference-inline", {
      builders: [code()],
    });
  ...
  it("having a code block results in inline styles are included", async () => {
    const sfc = await composeFixture("multi-code-block", {
      builders: [code()],
    });
  ...
  it("language is displayed when configured for it", async () => {
    ...
    const display = select(sfc.html).findFirst(
      ".lang-display",
      "didn't find .lang-display node!"
    );
  ...
  it.skip("whitespace at start of line is represented in final markup", async () => {
    const sfc = await composeFixture("ts-code-indented", {
      builders: [code()],
    });
    ...
    const sfc2 = await composeFixture("ts-code-indented", {
      builders: [code({ layoutStructure: "tabular" })],
    });
    const line22 = select(sfc2.html).findFirst(
      ".line-2",
      "couldn't find line 2"
    );
    const line23 = select(sfc2.html).findFirst(
      ".line-3",
      "couldn't find line 3"
    );
...
describe("table format for code blocks", () => {
  ...
  it("when tabular section is used along with heading and footer sections, this is visible too", async () => {
    ...
    const headingRow = body.findFirst(
      ".heading-row",
      "couldn't find .heading-row!"
    );
    const codeWrapper = body.findFirst(
      ".code-wrapper",
      "couldn't find .code-wrapper!"
    );
    const codeBlock = body.findFirst(
      ".code-block",
      "couldn't find .code-block!"
    );

    expect(headingRow).toBeTruthy();
    expect(getClassList(headingRow)).toContain("with-heading");
    expect(body.findFirst(".heading"), ".heading exists on page").toBeTruthy();
    expect(
