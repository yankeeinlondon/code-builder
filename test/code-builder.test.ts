...
describe("code() builder using Prism (incl generalized tests)", () => {
  it("valid language choice is rendered", async () => {
    const { templateBlock } = await composeFixture("multi-code-block.md", { builders: [code()] });
  ...
  it('"unknown language" fallback is used when language stated but not matched', async () => {
    const { templateBlock } = await composeFixture("unknown-code-block", { builders: [code()] });
  ...
  it("the correct fallback is used when NO language is found", async () => {
    const { templateBlock } = await composeFixture("no-language", { builders: [code({
  ...
  it("line numbers are displayed when set", async () => {
    const { html, templateBlock } = await composeFixture("ts-code-block", { builders: [code({
      lineNumbers: true  
    })] });
  ...
  it("line highlighting of line range works using object syntax", async () => {
    ...
    for (const el of highlighted)  {expect(getClassList(el)).toContain("code-line");}
    ...
    for (const el of highlighted2)  {expect(getClassList(el)).toContain("code-line");}
  });
  ...
  it("code content loaded from file using <<< syntax", async () => {
    const { html } = await composeFixture("external-reference", { builders: [code()] });
...
describe("table format for code blocks", () => {
  it("a code block can be converted to use a tabular HTML output", async () => {
    ...
    for (const l of codeLines)  {expect(l.tagName).toBe("TD");}
