...
export const parseObjectSyntax = (
  ...
  try {
      ...
      for (const h of highlight) {
        switch (typeof h) {
          ...
          case "number":
            ...
            break;
          case "object":
            if (isSymbolRepresentation(h)) {highlightTokens.push({ kind: "symbol", symbol: h.symbol });} else if (isObjectRangeRepresentation(h)) {highlightTokens.push({ kind: "range", from: h.from, to: h.to });} else {console.warn(`the "${p?.fileName}" file has a highlight token "${JSON.stringify(h)}" which is not parsable. This value will be ignored.`);}
    ...
    // other props
    if (hasProps(props)) {
      if (props.filename) {externalFile = props.filename;};
      if (props.showFilename) {showFilename = props.showFilename;};
    }
