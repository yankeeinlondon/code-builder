 ...
 */
export const mergeColorThemes = <
  T1 extends CodeColorTheme<any>,
  T2 extends CodeColorTheme<any>
>(
  light: T1,
  dark: T2
): CodeColorTheme<ColorByMode> => {
  const props = [...new Set([...Object.keys(light), ...Object.keys(dark)])];
  return props.reduce(
    (acc, prop) => ({
      ...acc,
      [prop]: [
